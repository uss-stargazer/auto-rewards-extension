...
const EXTENSION_JSON = path.join(SRC_DIR, "extension.json");

function updateManifestJson(contentScripts, background) {
  const manifest = JSON.parse(fs.readFileSync(MAINFEST_JSON, "utf8"));
  manifest["content_scripts"] = contentScripts;
  manifest["background"] = background;
  fs.writeFileSync(MAINFEST_JSON, JSON.stringify(manifest), "utf8");
}
  ...
  updateManifestJson(
    extensionData.contentScripts.map((contentScript) => {
      return {
        ...
        ].map((jsfile) => path.join(DIST_JS_DIR, jsfile)),
      };
    }),
    { service_worker: path.join(DIST_JS_DIR, "background.js") }
  );
